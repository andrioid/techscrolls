...
export class Bayes implements ClassifierOptions {
  ...
  static defaultTokenizer(text: string) {
    ...
    var sanitized = text.toLowerCase().replaceAll(sanitizeText, "");
    const split = sanitized.split(/\s+/);
    const arr = split.filter((sc) => {
      const s = sc.replaceAll(sanitizeToken, "");
      if (s === "") return false;
      if (s.length <= 2) return false;
      if (s.match(/^\d+$/)) return false; // numbers
      if (stopwords.en.includes(s)) return false;
      if (s.match(/^@\w+/)) return false; // mentions
      if (s.match(/\w+\.\w+\/\w+/)) return false; // url like words
...
const sanitizeText = new RegExp(/[^a-z0-9\s\.\/]/gi);
const sanitizeToken = new RegExp(/[\.]$/gi);
